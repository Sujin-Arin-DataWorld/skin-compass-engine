  ...
  <div className="flex gap-1.5 flex-wrap sm:flex-nowrap">
    {[0, 1, 2, 3].map((v) => (
      <motion.button
        key={v}
        onClick={() => onChange(v)}
        className={`relative flex-1 min-h-[44px] min-w-[70px] rounded-lg px-2 py-2.5 text-xs font-medium transition-all overflow-hidden select-none touch-manipulation border ${
          value === v
            ? v === 0
      ...
      >
        ...
        <span className="relative z-10 whitespace-nowrap">{SEVERITY_LABELS[v]}</span>
      </motion.button>
    ))}
  </div>
);
